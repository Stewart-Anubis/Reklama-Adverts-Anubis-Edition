...
{
	name = "Reklama-Adverts",
	author = "Pheonix, Anubis edition",
	version = "1.4.1-A",
	url = ""
};
...
{
	if(GetEngineVersion() != Engine_CSGO) SetFailState("[Adverts] - plugin only for CSGO server");
	KFG_load();
...
void KFG_load()
{
	...
	if(tim) delete tim;
	kfg = new KeyValues("adverts");
	static char path[128], h[1024], buf[64];
	if(!path[0]) BuildPath(Path_SM, path, 128, "configs/reklama.cfg");
	if(!kfg.ImportFromFile(path)) SetFailState("[Adverts] - Configuration file not found");
	else
	{
		kfg.Rewind();
...
void Replese_constant(char[] rkl, Rtupe tupe)
{
	...
	switch (tupe)
	{
		case C, H:
		{
			ReplaceString(rkl, 1024, "{{", "<font color='");
			ReplaceString(rkl, 1024, "}}", "</font>");
			ReplaceString(rkl, 1024, "==", "'>");
		}
